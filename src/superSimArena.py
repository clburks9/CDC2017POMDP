...
def runMultiSim(think,step=-1,simCount=10,simSteps = 100,alphaNum = 2,useSoft=False,MCTS = False,greedy = False):
	#run simulations
	allSimRewards = [];
	allSimAct = []; 
	allSimX = []; 
	...
	if(useSoft):
		if(step==-1):
			...
			policy = np.load("../policies/D4DiffsSoftmax/D4DiffsSoftmaxAlphas"+think+"Step"+str(step)+".npy",encoding='latin1');
			modelModule = __import__('D4DiffsSoftmaxModel', globals(), locals(), ['ModelSpec'],0); 
			modelClass = modelModule.ModelSpec;
			modelName = 'D4DiffsSoftmax'
	else:
		if(step==-1):
			...
			policy = np.load("../policies/D4Diffs/D4DiffsAlphas"+think+"Step"+str(step)+".npy",encoding='latin1');
			modelModule = __import__('D4DiffsModel', globals(), locals(), ['ModelSpec'],0); 
			modelClass = modelModule.ModelSpec;
			modelName = 'D4Diffs'
	...
	allModActs = modelClass(mode=think); 
