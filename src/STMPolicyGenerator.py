...
Incorporates State Transition Matrixes
				...
				for o in range(0,len(pz)):
					...
					for k in range(0,G[j].size):
						for l in range(0,pz[o].size): 
							...
							smean = (np.transpose(np.matrix(ss)) - np.matrix(self.delA[a]))
							...
							als1[j][a][o].addG(Gaussian(meanPrime,varPrime,weightPrime)); 
		self.preAls = als1; 
		...
		for j in range(0,len(G)):
			for a in range(0,len(self.delA)):
				for o in range(0,self.pz.size):
					...
					for k in alObs.Gs:
						mean = (np.matrix(k.mean) - np.matrix(self.delA[a]))
						var = (np.matrix(k.var) + np.matrix(self.delAVar))
						weight = k.weight; 
